...
__global__ void PSAlignPoolingForward(
    const int nthreads,  const Dtype* bottom_data,  const Dtype spatial_scale,
    const int channels, const int height, const int width,
    const int pooled_height, const int pooled_width,
    const int sample_height, const int sample_width,
    const Dtype* bottom_rois, const int output_dim,  const int group_size,
    Dtype* top_data,  int* mapping_channel,  int* argmax_position) {
    CUDA_KERNEL_LOOP(index, nthreads){
        int n = index; // output_dim==channels
        int ctop = n % output_dim;//int ctop = (index / pooled_width / pooled_height) % output_dim;
        n /= output_dim;
        int pw = n % pooled_width; //int pw = index % pooled_width;
        n /= pooled_width;
        int ph = n % pooled_height;//int ph = (index / pooled_width) % pooled_height;
        n /= pooled_height;        //int n = index / pooled_width / pooled_height / output_dim;
        bottom_rois += n * 5;
        ...
        float hcenter, wcenter;
        int c = (ctop*group_size + ph)*group_size + pw; //////////////////////////
        float tmp = float(-1e20), tmp2; // == maxval
        int buf_value = -1;             // == maxidx    //////////////////////////
