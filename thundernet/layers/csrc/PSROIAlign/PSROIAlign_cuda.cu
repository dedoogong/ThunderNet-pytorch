...
__global__ void RoIAlignForward(
    ...
    const int sampling_ratio, const T* bottom_rois, const int group_size, T* top_data, int* argmax_position, bool aligned) {
  CUDA_1D_KERNEL_LOOP(index, nthreads) {    // (n, c, ph, pw) is an element in the pooled output
    ...
    if (!aligned) { roi_width  = max(roi_width, (T)1.);
                    roi_height = max(roi_height,(T)1.); } // for backward-compatibility only
    T bin_size_h = static_cast<T>(roi_height)/ static_cast<T>(pooled_height);
    T bin_size_w = static_cast<T>(roi_width) / static_cast<T>(pooled_width);
    //---------------------------- start ---------------------------- //
    int c_ = (pw*group_size + ph)*group_size + c;
    const T* offset_bottom_data = bottom_data + (roi_batch_ind * channels + c_) * height * width;
...
at::Tensor ROIAlign_forward_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "ROIAlign_forward", [&] {
    RoIAlignForward<scalar_t><<<grid, block, 0, stream>>>(
        output_size,                            //////// const int nthreads,
        input.contiguous().data_ptr<scalar_t>(),//////// const T* bottom_data,
        spatial_scale,                          //////// const T spatial_scale,
        channels,
        height,
        width,
        pooled_height,
        pooled_width,
        sampling_ratio,
        rois.contiguous().data_ptr<scalar_t>(), /////// const T* bottom_rois,
